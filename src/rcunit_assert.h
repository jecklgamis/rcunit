...
#define RCU_ASSERT_EQUAL_PTR(expected, actual) \
...
#define RCU_ASSERT_NOT_EQUAL_PTR(expected, actual) \
 { void *__expected = (void *)(expected); void *__actual = (void *)(actual); \
...
#define RCU_ASSERT_LT(a, b) \
 { long __a = (long)(a); long __b = (long)(b); \
   rcu_assert_impl(__a < __b, __FILE__,__func__,__LINE__, \
   "%s (%ld) is not less than %s (%ld)", #a, __a, #b, __b); }

#define RCU_ASSERT_GE(a, b) \
 { long __a = (long)(a); long __b = (long)(b); \
   rcu_assert_impl(__a >= __b, __FILE__,__func__,__LINE__, \
   "%s (%ld) is not >= %s (%ld)", #a, __a, #b, __b); }

#define RCU_ASSERT_LE(a, b) \
 { long __a = (long)(a); long __b = (long)(b); \
   rcu_assert_impl(__a <= __b, __FILE__,__func__,__LINE__, \
   "%s (%ld) is not <= %s (%ld)", #a, __a, #b, __b); }
...
#define RCU_ASSERT_EQUAL_FLOAT(expected, actual) \
...
#define RCU_ASSERT_NOT_EQUAL_FLOAT(expected, actual) \
    { double __expected = expected; double __actual = actual; \
...
/* String assertions */
#define RCU_ASSERT_EQUAL_STR(expected, actual) \
...
#define RCU_ASSERT_NOT_EQUAL_STR(expected, actual) \
 { const char *__expected = (expected); const char *__actual = (actual); \
...
#define RCU_ASSERT_STR_CONTAINS(haystack, needle) \
...
#define RCU_ASSERT_STR_EMPTY(str) \
 { const char *__str = (str); \
   rcu_assert_impl(__str != NULL && __str[0] == '\0', __FILE__,__func__,__LINE__, \
   "%s is not empty", #str); }

#define RCU_ASSERT_STR_NOT_EMPTY(str) \
 { const char *__str = (str); \
   rcu_assert_impl(__str != NULL && __str[0] != '\0', __FILE__,__func__,__LINE__, \
   "%s is empty", #str); }

/* Memory and array assertions */
#define RCU_ASSERT_EQUAL_MEM(a, b, size) \
    { rcu_assert_impl( !(memcmp(a, b, size)), __FILE__,__func__, __LINE__, "memory regions are not equal"); }

#define RCU_ASSERT_NOT_EQUAL_MEM(a, b, size) \
    { rcu_assert_impl( (memcmp(a, b, size)), __FILE__,__func__, __LINE__, "memory regions are equal"); }

#define RCU_ASSERT_ZERO_MEM(ptr, size) \
...
#define RCU_ASSERT_EQUAL_ARRAY(expected, actual, n) \
